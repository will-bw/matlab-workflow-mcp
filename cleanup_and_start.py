...
解决问题: Python 进程异常退出（如 OOM）后，MATLAB Engine 子进程可能残留，
...
def kill_matlab_processes():
    """清理本服务残留的 MATLAB 进程（仅清理无控制台窗口的后台引擎进程）"""
    killed = 0

    if sys.platform == "win32":
        try:
            # 仅查找由 MATLAB Engine API 启动的进程（无控制台窗口）
            # 使用 wmic 查找 CommandLine 包含 "-automation" 的 MATLAB 进程
            ...
            for line in result.stdout.splitlines():
                # Engine API 启动的 MATLAB 进程命令行包含 -automation 标志
            ...
            if engine_pids:
                logger.info(f"检测到 {len(engine_pids)} 个残留 MATLAB Engine 进程: {engine_pids}")
                ...
                if "MATLAB.exe" in result.stdout:
                    logger.warning(
                        "检测到 MATLAB 进程但无法确认是否为 Engine 进程。"
        ...
        # Linux/macOS: 仅杀 matlabengine 相关进程
        try:
            result = subprocess.run(
                ["pgrep", "-f", "matlab.*-automation"],
                capture_output=True, text=True, timeout=5
            )
            if result.stdout.strip():
                logger.info("检测到残留 MATLAB Engine 进程，正在清理...")
                subprocess.run(["pkill", "-9", "-f", "matlab.*-automation"], timeout=10)
                killed += 1
                time.sleep(2)
        except Exception as e:
            logger.warning(f"清理 MATLAB 进程时出错: {e}")
    ...
    if killed > 0:
        logger.info(f"已清理 {killed} 个残留 MATLAB Engine 进程")
    else:
        logger.info("无残留 MATLAB Engine 进程")
...
def main():
    ...
    # Step 1: 清理残留进程
    logger.info("[1/2] 检查并清理残留 MATLAB Engine 进程...")
